...
- **Top Churn Reason:** Competitor switching (36%), followed closely by low engagement (32%) and inactivity (32%).
- **Promotion Inefficacy:** Promotions have **zero impact** on basket size. Average spend is identical with (Rs. 516.30) and without (Rs. 516.23) a discount.
- **Loyalty Paradox:** The **Elite tier** has the highest churn rate (55%), while the **Basic tier** has the lowest (41%).
- **High-Value At-Risk Segment:** A distinct group of "High CLV" customers has recently gone inactive. These are the most critical targets for win-back campaigns.
...
### Data Model
The solution utilizes a **Star Schema** with `Customer_Demographics` as the central fact table.
...
- `(*)` Customer_Transactions `(1)` Store_Locations (via `Store_ID`)
...
This section identifies **who** is leaving and **why**.
...
- **Store Format:** Churn is exactly 50% across all store types (Supercenter, Sam's Club, etc.), indicating a systemic issue rather than a location-specific one.
